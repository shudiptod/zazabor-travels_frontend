...
const SinglePackage = (props) => {
    ...
    return (
        <div className="container mx-auto lg:w-5/6 md:w-6/6 sm:w-5/6 h-full bg-gray-50 filter drop-shadow shadow-2xl border rounded-xl">
            ...
            <div className="container mx-auto w-full px-8 py-2 flex flex-col">
                <h3 className="text-lg text-left text-gray-900 font-mono mb-5">{name}</h3>
                <div className="w-full flex justify-between">
                    <h3 className="text-md text-left text-gray-900 font-mono ">{people} Person</h3>
                    <h3 className="text-md text-left text-gray-900 font-mono ">{duration} days</h3>
                </div>
                <p className="text-sm text-justify font-serif font-semibold text-gray-800 mb-2 w-10/12">{description.substring(0, 100) + '...'}</p>
                <Link to={'/booking/' + key}>
