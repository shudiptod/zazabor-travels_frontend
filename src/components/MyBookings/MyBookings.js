...
const MyBookings = () => {
  ...
  useEffect(() => {
    fetch(
      `https://zazabor-travelsbackend-production.up.railway.app/bookings?email=${email}`
    )
      .then((res) => res.json())
      .then((data) => {
        // console.log(data);
  ...
  return (
    <div>
      <h2 className="text-center text-blue-900 font-semibold font-mono text-2xl my-5">
        Your Booking List
      </h2>
      {bookings.length === 0 ? (
