...
const SingleBooking = (props) => {
    ...
    const handleUpdate = () => {
        fetch(`https://pacific-castle-78944.herokuapp.com/updatePending?=${props.booking._id}`, {
            method: "PUT",
            headers: {
        ...
        <div className="flex mt-4 ">
            ...
            <div className="w-6/12 border-l-4 border-gray-800 flex flex-col">
                ...
                <div className="flex w-8/12 mx-auto justify-around mt-3">
                    <button onClick={() => handleDelete()} className="px-4 py-1 bg-yellow-600 text-white font-semibold rounded-md">{
                    ...
                    }</button>
                    {
                        url === "/allBookings" ?
                            <>
                                {
                                    pending ? <button onClick={() => handleUpdate()}
                                        className="px-4 py-1 bg-green-700 text-white font-semibold rounded-md">
                                        Approve Booking</button> : <button onClick={() => handleUpdate()}
